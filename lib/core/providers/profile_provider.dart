...
class ProfileProvider extends ChangeNotifier {
  ...
  CachedProfile? getCachedProfile(String userId) {
    ...
    if (cached != null && cached.isValid) {
      debugPrint(
          '✅ [ProfileProvider] Using cached profile for userId: $userId');
      return cached;
    }
    return null;
  }
  ...
  Future<void> loadProfile(String userId,
      {bool forceRefresh = false, BuildContext? context}) async {
    final cached = getCachedProfile(userId);

    // If forcing refresh, skip all cache checks
    if (forceRefresh) {
      debugPrint(
          '🔄 [ProfileProvider] Force refresh requested for userId: $userId');
    ...
    if (cached == null) {
      debugPrint(
          '🌐 [ProfileProvider] No cache found, fetching fresh data for userId: $userId');
      await _fetchProfile(userId, context);
      return;
    }
    ...
    if (!cached.isValid) {
      debugPrint(
          '⏰ [ProfileProvider] Cache expired (> 5 min), fetching fresh data for userId: $userId');
      await _fetchProfile(userId, context);
      return;
    }
    ...
    debugPrint(
        '🔍 [ProfileProvider] Checking for server updates for userId: $userId');
    final hasUpdates = await _checkForServerUpdates(userId, cached, context);

    if (hasUpdates) {
      debugPrint(
          '🆕 [ProfileProvider] Server has updates, fetching fresh data for userId: $userId');
      await _fetchProfile(userId, context);
    } else {
      debugPrint(
          '✅ [ProfileProvider] Using cached profile (no server changes) for userId: $userId');
  ...
  Future<bool> _checkForServerUpdates(
      String userId, CachedProfile cached, BuildContext? context) async {
    try {
      ...
      if (serverPostCount != cached.postCount) {
        debugPrint(
            '📊 [ProfileProvider] Post count changed: ${cached.postCount} → $serverPostCount');
        return true;
      }
      ...
      if (profileResult['success'] == true && profileResult['data'] != null) {
        ...
        if (data['followers'] != null && cached.profile != null) {
          ...
          if (serverFollowersCount != cachedFollowersCount) {
            debugPrint(
                '👥 [ProfileProvider] Followers changed: $cachedFollowersCount → $serverFollowersCount');
            return true;
          }
        }
        ...
        if (data['following'] != null && cached.profile != null) {
          ...
          if (serverFollowingCount != cachedFollowingCount) {
            debugPrint(
                '👤 [ProfileProvider] Following changed: $cachedFollowingCount → $serverFollowingCount');
            return true;
          }
        }
      }
      ...
      debugPrint(
          '⚠️ [ProfileProvider] Error checking for updates, will use cache: $e');
  ...
  Future<void> _fetchProfile(String userId, BuildContext? context) async {
    ...
    try {
      ...
      final results = await Future.wait([
        ...
        _profileService.getUserThoughtPosts(userId),
      ]);
  ...
  Future<void> refreshProfile(String userId, {BuildContext? context}) async {
    debugPrint(
        '🔄 [ProfileProvider] Force refreshing profile for userId: $userId');
