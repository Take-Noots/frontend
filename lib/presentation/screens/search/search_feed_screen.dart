                              ...
                              children: [
                                ...
                                if ((_searchResults['fanbases'] ?? [])
                                  ...
                                  FanbaseSearchResults(
                                    fanbases: (_searchResults['fanbases'] ?? [])
                                        as List,
                                    query: _query,
                                    shrinkWrap: true,
                                    physics:
                                ...
                                if ((_searchResults['posts'] ?? []).isNotEmpty)
                                  ListView.separated(
                                    ...
                                    physics:
                                        const NeverScrollableScrollPhysics(),
                                    itemCount:
                                        (_searchResults['posts'] ?? []).length,
                                    separatorBuilder: (context, index) =>
                                        const SizedBox(height: 8),
                                    itemBuilder: (context, index) {
                                      final post = (_searchResults['posts'] ??
                                          [])[index];
                                      return DesPost.Post(
                                        ...
                                        onComment: () =>
                                            print('Comment: ${post['_id']}'),
                                        onShare: () =>
                                            print('Share: ${post['_id']}'),
                                        onPlayPause: () =>
                                            print('Play/Pause: ${post['_id']}'),
                                        onUsernameTap: () => print(
                                            'Username tap: ${post['username']}'),
                                        isLiked: false,
                                        isPlaying: false,
                                ...
                                if ((_searchResults['songPosts'] ?? [])
                                    .isNotEmpty)
                                  FeedWidget(
                                    feedItems: (_searchResults['songPosts'] ??
                                            [])
                                        .map<FeedItem>((post) => FeedItem.song(
                                            data_model.Post.fromJson(post)))
                                        .toList(),
                                    isLoading: false,
                                    error: null,
                                    onRefresh: () {},
                                    onSongLike: (post) =>
                                        print('Liked song post: ${post.id}'),
                                    onSongComment: (post) =>
                                        print('Comment song post: ${post.id}'),
                                    onSongPlay: (post) => print(
                                        'Play/Pause song post: ${post.id}'),
                                    onSongShare: (post) =>
                                        print('Share song post: ${post.id}'),
                                    ...
                                    builder: (context) {
                                      final songPosts =
                                          (_searchResults['songPosts'] ?? [])
                                              as List;
                                      print(
                                          'Song Posts data from search results: $songPosts');
                                      ...
                                      return FeedWidget(
                                        feedItems: songPosts
                                            .map<FeedItem>((post) =>
                                                FeedItem.song(
                                                    data_model.Post.fromJson(
                                                        post)))
                                            .toList(),
                                        isLoading: false,
                                        error: null,
                                        onRefresh: () {},
                                        onSongLike: (post) => print(
                                            'Liked song post: ${post.id}'),
                                        onSongComment: (post) => print(
                                            'Comment song post: ${post.id}'),
                                        onSongPlay: (post) => print(
                                            'Play/Pause song post: ${post.id}'),
                                        onSongShare: (post) => print(
                                            'Share song post: ${post.id}'),
                                        currentlyPlayingTrackId: null,
                                        isPlaying: false,
                                        onUserTap:
                                            (String userId, String? username) =>
